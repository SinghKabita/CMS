...
! function($) {
    var calenderData = {
            ...
            extractedBsMonthData: [
                ...
                [29, 1, 26, 1, 28, 1, 2, 1, 2],
                [1, 1, 3, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 3, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 3, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 5, 1, 1, 2, 2],
        ...
        validationFunctions = {
            ...
            validateBsYear: function(bsYear) {
                ...
                if (bsYear < calenderData.minBsYear || bsYear > calenderData.maxBsYear) throw new RangeError("Parameter bsYear value should be in range of " + calenderData.minBsYear + " to " + calenderData.maxBsYear)
            },
            validateBsMonth: function(bsMonth) {
                ...
                if (0 > bsMonth || bsMonth > 11) throw new RangeError("Parameter bsMonth value should be in range of 0 to 11")
            },
            validateBsDate: function(bsDate) {
                if ("number" != typeof bsDate || null === bsDate) throw new TypeError("Invalid parameter bsDate value");
                if (1 > bsDate || bsDate > 32) throw new RangeError("Parameter bsDate value should be in range of 1 to 32")
            },
            validatePositiveNumber: function(numberParameters) {
                $.each(numberParameters, function(key, value) {
                    if ("number" != typeof value || null === value || 0 > value) throw new ReferenceError("Invalid parameters: " + Object.keys(numberParameters).join(", "));
                    if ("yearDiff" === key && value > calenderData.maxBsYear - calenderData.minBsYear + 1) throw new RangeError("Parameter yearDiff value should be in range of 0 to " + (calenderData.maxBsYear - calenderData.minBsYear + 1))
                })
            }
        };
    $.extend(calenderFunctions, {
        getNepaliNumber: function(number) {
            ...
            if ("number" != typeof number || 0 > number) throw new Error("Number should be positive integer");
            var prefixNum = Math.floor(number / 10),
                suffixNum = number % 10;
            return 0 !== prefixNum ? calenderFunctions.getNepaliNumber(prefixNum) + calenderData.nepaliNumbers[suffixNum] : calenderData.nepaliNumbers[suffixNum]
        },
        getNumberByNepaliNumber: function(nepaliNumber) {
            ...
            for (var number = 0, i = 0; i < nepaliNumber.length; i++) {
                ...
                if (-1 === numIndex) throw new Error("Invalid nepali number");
        ...
        getBsMonthInfoByBsDate: function(bsYear, bsMonth, bsDate, dateFormatPattern) {
            ...
            var daysNumFromMinBsYear = calenderFunctions.getTotalDaysNumFromMinBsYear(bsYear, bsMonth, bsDate),
                adDate = new Date(calenderData.minAdDateEqBsDate.ad.year, calenderData.minAdDateEqBsDate.ad.month, calenderData.minAdDateEqBsDate.ad.date - 1);
            ...
            var bsMonthFirstAdDate = calenderFunctions.getAdDateByBsDate(bsYear, bsMonth, 1),
                bsMonthDays = calenderFunctions.getBsMonthDays(bsYear, bsMonth);
            bsDate = bsDate > bsMonthDays ? bsMonthDays : bsDate;
            var eqAdDate = calenderFunctions.getAdDateByBsDate(bsYear, bsMonth, bsDate),
                weekDay = eqAdDate.getDay(),
                formattedDate = calenderFunctions.bsDateFormat(dateFormatPattern, bsYear, bsMonth, bsDate, weekDay);
            return {
                bsYear: bsYear,
                bsMonth: bsMonth,
                bsDate: bsDate,
        ...
        getTotalDaysNumFromMinBsYear: function(bsYear, bsMonth, bsDate) {
            if (bsYear < calenderData.minBsYear || bsYear > calenderData.maxBsYear) return null;
            for (var daysNumFromMinBsYear = 0, diffYears = bsYear - calenderData.minBsYear, monthIndex = 0; 12 > monthIndex; monthIndex++) daysNumFromMinBsYear += bsMonth > monthIndex ? calenderFunctions.getMonthDaysNumFormMinBsYear(monthIndex, diffYears + 1) : calenderFunctions.getMonthDaysNumFormMinBsYear(monthIndex, diffYears);
            return daysNumFromMinBsYear += bsYear > 2085 && 2088 > bsYear ? bsDate - 2 : bsYear > 2088 && bsMonth > 4 ? bsDate - 4 : bsDate
        },
        getMonthDaysNumFormMinBsYear: function(bsMonth, yearDiff) {
            ...
            for (var bsMonthData = calenderData.extractedBsMonthData[bsMonth], i = 0; i < bsMonthData.length; i++)
                if (0 !== bsMonthData[i]) {
                    var bsMonthUpperDaysIndex = i % 2;
                    if (!(yearDiff > yearCount + bsMonthData[i])) {
                        monthDaysFromMinBsYear += calenderData.bsMonthUpperDays[bsMonth][bsMonthUpperDaysIndex] * (yearDiff - yearCount), yearCount = yearDiff - yearCount;
                        break
                    }
                    yearCount += bsMonthData[i], monthDaysFromMinBsYear += calenderData.bsMonthUpperDays[bsMonth][bsMonthUpperDaysIndex] * bsMonthData[i]
                }
        ...
        getBsMonthDays: function(bsYear, bsMonth) {
            ...
            for (var yearCount = 0, totalYears = bsYear + 1 - calenderData.minBsYear, bsMonthData = calenderData.extractedBsMonthData[bsMonth], i = 0; i < bsMonthData.length; i++)
                if (0 !== bsMonthData[i]) {
                    var bsMonthUpperDaysIndex = i % 2;
                    if (yearCount += bsMonthData[i], yearCount >= totalYears) return 2085 == bsYear && 4 == bsMonth || 2088 == bsYear && 4 == bsMonth ? calenderData.bsMonthUpperDays[bsMonth][bsMonthUpperDaysIndex] - 2 : calenderData.bsMonthUpperDays[bsMonth][bsMonthUpperDaysIndex]
                }
            return null
        },
        getBsDateByAdDate: function(adYear, adMonth, adDate) {
            ...
            if (adDate >= 1 && adDate < bsMonthFirstAdDate.getDate()) {
                bsMonth = 0 !== bsMonth ? bsMonth - 1 : 11;
        ...
        var datePickerPlugin = {
            ...
            init: function($element) {
                ...
                $("body").append($nepaliDatePicker), "" !== $element.val() ? datePickerPlugin.renderFormattedSpecificDateCalender($nepaliDatePicker, datePickerPlugin.options.dateFormat, $element.val()) : datePickerPlugin.renderCurrentMonthCalender($nepaliDatePicker), datePickerPlugin.addEventHandler($element, $nepaliDatePicker)
            },
            addCommonEventHandler: function() {
                var $datePickerWrapper = $(".nepali-date-picker");
                $(document).click(function(event) {
                    var $targetElement = $(event.target);
                    $targetElement.parents($(".nepali-date-picker")).length || ($datePickerWrapper.hide(), $datePickerWrapper.find(".drop-down-content").hide())
                })
            },
            addEventHandler: function($element, $nepaliDatePicker) {
                $element.focus(function() {
                    ...
                    }), $nepaliDatePicker.show(), datePickerPlugin.eventFire($element, $nepaliDatePicker, "show")
                    ...
                    datePickerPlugin.renderNextMonthCalender($nepaliDatePicker), datePickerPlugin.triggerChangeEvent($element, $nepaliDatePicker, preCalenderData), $nepaliDatePicker.show()
                    ...
                    datePickerPlugin.triggerChangeEvent($element, $nepaliDatePicker, preCalenderData), $nepaliDatePicker.show()
                    ...
                    if (!$(this).hasClass("disable")) {
                        var datePickerData = $nepaliDatePicker.data(),
                            ...
                            dateText = calenderFunctions.bsDateFormat(datePickerPlugin.options.dateFormat, bsYear, bsMonth, bsDate, weekDay);
                        $element.val(dateText), datePickerPlugin.setCalenderDate($nepaliDatePicker, bsYear, bsMonth, bsDate), datePickerPlugin.renderMonthCalender($nepaliDatePicker), datePickerPlugin.eventFire($element, $nepaliDatePicker, "select"), preDate != bsDate && datePickerPlugin.eventFire($element, $nepaliDatePicker, "dateChange"), datePickerPlugin.options.closeOnDateSelect ? $nepaliDatePicker.hide() : $nepaliDatePicker.show()
                    }
                    ...
                    var preCalenderData = {
                        ...
                        bsDate = preCalenderData.bsDate;
                    datePickerPlugin.setCalenderDate($nepaliDatePicker, bsYear, bsMonth, bsDate), datePickerPlugin.renderMonthCalender($nepaliDatePicker);
                    $nepaliDatePicker.data();
                    datePickerPlugin.triggerChangeEvent($element, $nepaliDatePicker, preCalenderData), $nepaliDatePicker.show()
                })
            },
            triggerChangeEvent: function($element, $nepaliDatePicker, preCalenderData) {
                ...
                preCalenderData.bsYear != calenderData.bsYear && datePickerPlugin.eventFire($element, $nepaliDatePicker, "yearChange"), preCalenderData.bsMonth != calenderData.bsMonth && datePickerPlugin.eventFire($element, $nepaliDatePicker, "monthChange"), preCalenderData.bsDate != calenderData.bsDate && datePickerPlugin.eventFire($element, $nepaliDatePicker, "dateChange")
            },
            ...
            renderMonthCalender: function($nepaliDatePicker) {
                $nepaliDatePicker.find(".calender-wrapper").remove(), $nepaliDatePicker.append(datePickerPlugin.getCalender($nepaliDatePicker)).hide()
            },
            ...
            getYearDropOption: function($nepaliDatePicker) {
                ...
                $yearSpan.text(calenderFunctions.getNepaliNumber(datePickerData.bsYear)), $yearSpan.append('<i class="icon icon-drop-down">');
            ...
            getCalenderHeader: function() {
                for (var calenderHeader = $("<thead>"), tableRow = $("<tr>"), i = 0; 7 > i; i++) tableRow.append("<td>" + calenderData.bsDays[i] + "</td>");
                return calenderHeader.append(tableRow), calenderHeader
            },
            getCalenderBody: function($nepaliDatePicker) {
                var datePickerData = $nepaliDatePicker.data(),
                    ...
                    preMonth = datePickerData.bsMonth - 1 != -1 ? datePickerData.bsMonth - 1 : 11,
                    preYear = 11 == preMonth ? datePickerData.bsYear - 1 : datePickerData.bsYear,
                    preMonthDays = calenderFunctions.getBsMonthDays(preYear, preMonth),
                    minBsDate = null,
                    maxBsDate = null;
                null !== datePickerPlugin.options.minDate && (minBsDate = calenderFunctions.parseFormattedBsDate(datePickerPlugin.options.dateFormat, datePickerPlugin.options.minDate)), null !== datePickerPlugin.options.maxDate && (maxBsDate = calenderFunctions.parseFormattedBsDate(datePickerPlugin.options.dateFormat, datePickerPlugin.options.maxDate));
                for (var calenderBody = $("<tbody>"), i = 0; weekCoverInMonth > i; i++) {
                    for (var tableRow = $("<tr>"), k = 1; 7 >= k; k++) {
                        var calenderDate = 7 * i + k - datePickerData.bsMonthFirstAdDate.getDay(),
                            isCurrentMonthDate = !0;
                        if (0 >= calenderDate ? (calenderDate = preMonthDays + calenderDate, isCurrentMonthDate = !1) : calenderDate > datePickerData.bsMonthDays && (calenderDate -= datePickerData.bsMonthDays, isCurrentMonthDate = !1), isCurrentMonthDate) {
                            ...
                            calenderDate == datePickerData.bsDate && $td.addClass("active"), datePickerPlugin.disableIfOutOfRange($td, datePickerData, minBsDate, maxBsDate, calenderDate), tableRow.append($td)
                        } else tableRow.append('<td class="other-month-date">' + calenderFunctions.getNepaliNumber(calenderDate) + "</td>")
                    }
            ...
            renderCurrentMonthCalender: function($nepaliDatePicker) {
                var currentDate = new Date,
                    currentBsDate = calenderFunctions.getBsDateByAdDate(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()),
            ...
            renderPreviousMonthCalender: function($nepaliDatePicker) {
                var datePickerData = $nepaliDatePicker.data(),
                    prevMonth = datePickerData.bsMonth - 1 >= 0 ? datePickerData.bsMonth - 1 : 11,
                    prevYear = 11 != prevMonth ? datePickerData.bsYear : datePickerData.bsYear - 1,
                    prevDate = datePickerData.bsDate;
                return prevYear < datePickerPlugin.options.yearStart || prevYear > datePickerPlugin.options.yearEnd ? null : (datePickerPlugin.setCalenderDate($nepaliDatePicker, prevYear, prevMonth, prevDate), void datePickerPlugin.renderMonthCalender($nepaliDatePicker))
            },
            renderNextMonthCalender: function($nepaliDatePicker) {
                var datePickerData = $nepaliDatePicker.data(),
                    nextMonth = datePickerData.bsMonth + 1 <= 11 ? datePickerData.bsMonth + 1 : 0,
                    nextYear = 0 !== nextMonth ? datePickerData.bsYear : datePickerData.bsYear + 1,
                    nextDate = datePickerData.bsDate;
                return nextYear < datePickerPlugin.options.yearStart || nextYear > datePickerPlugin.options.yearEnd ? null : (datePickerPlugin.setCalenderDate($nepaliDatePicker, nextYear, nextMonth, nextDate), void datePickerPlugin.renderMonthCalender($nepaliDatePicker))
            },
            renderFormattedSpecificDateCalender: function($nepaliDatePicker, dateFormat, dateFormattedText) {
                ...
                datePickerPlugin.setCalenderDate($nepaliDatePicker, datePickerDate.bsYear, datePickerDate.bsMonth, datePickerDate.bsDate), datePickerPlugin.renderMonthCalender($nepaliDatePicker)
            }
        };
